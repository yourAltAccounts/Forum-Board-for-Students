...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		String errMessagePassword = ModelNewAccount.evaluatePassword(password);
		
		// Clear the input widgets and display error message if one of the checks returns invalid
		if (errMessageUserName != "") {
			...
			Label usernameError = new Label(errMessageUserName);
			usernameError.setWrapText(true);
			
			ViewNewAccount.alertUsernamePasswordValidity.getDialogPane().setContent(usernameError);
			ViewNewAccount.alertUsernamePasswordValidity.showAndWait();
		} else if (errMessagePassword != "") {
			ViewNewAccount.text_Username.setText("");
			ViewNewAccount.text_Password1.setText("");
			ViewNewAccount.text_Password2.setText("");
			Label passwordError = new Label(errMessagePassword);
			passwordError.setWrapText(true);

			ViewNewAccount.alertUsernamePasswordValidity.getDialogPane().setContent(passwordError);
			ViewNewAccount.alertUsernamePasswordValidity.showAndWait();
		} else {
			valid = true;
		}
		...
		if (ViewNewAccount.text_Password1.getText().
				compareTo(ViewNewAccount.text_Password2.getText()) == 0 && valid == true) {
